...
namespace ConsoleApp1
{
    class Program
    {
        ...
        // propriété Réalise une action(calcul) qui est ensuite retourner sur myVar
        private int myVar;
        ...
        // autre façon d'écrire une propriété
        public int myVar => 1 + 1;

        // static stocke une methode utilisable partout.

        static var item = "static";


        //Récupère un objet sans connaitre le type.
        public T MyGeneric<T>(T item)
        {
            return item;
        }

        // event; active une méthode prè-enregistrée(ex: appui sur un bouton). Action est un type générique.
        public event Action MyAction;


        Myaction += myMethod;   // abonne une méthode a l'event.
        MyAction -= myMethod;  // la désabonne.
        if (MyAction != null)
        { // vérifie si l'event a des abonnés.
         MyAction.Invoke();   // déclenche l'event.
        }
    // Evénement qui fonctionne comme les autres event sauf qu'il fait apparaitre un nouvel objet sur l'inspecteur d'Unity. Permet de configurer les events dans l'inspecteur.
